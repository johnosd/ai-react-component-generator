...
// Mock ToolInvocationBadge so MessageList tests don't depend on its label logic
vi.mock("../ToolInvocationBadge", () => ({
  ToolInvocationBadge: ({ toolName, args }: { toolName: string; args: Record<string, unknown> }) => (
    <div data-testid="tool-invocation-badge">{`${toolName}:${args.command ?? ""}`}</div>
  ),
}));
...
test("MessageList renders ToolInvocationBadge for tool-invocation parts with create command", () => {
  const messages: Message[] = [
    {
      id: "1",
      role: "assistant",
  ...
  // The mock renders toolName:command, verifying props are forwarded correctly
  expect(screen.getByText("str_replace_editor:create")).toBeDefined();
});
